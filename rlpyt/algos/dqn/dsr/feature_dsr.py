...
TCFOptInfo = namedtuple("TCFOptInfo", ["posDistance", "negDistance"] + list(FeatureOptInfo._fields) + list(SFeatureOptInfo._fields))
...
class LandmarkTCFDSR(FeatureDSR):
    ...
    def samples_to_buffer(self, samples, encode=False):
    ...
    def example_to_buffer(self, examples, encode=False):
        return SamplesToBuffer(
    ...
    def dsr_loss(self, samples):
        ...
        with torch.no_grad():
            ...
            target_features = self.agent.encode(samples.target_inputs.observation) # B x F, s_{t'} --> phi_{t'}

            # 2b. estimate target successor features given features
            target_dsr = self.agent.target(target_features) # B x A x F, phi_{t'} --> psi_{t'}

            target_s_features = torch.mean(target_dsr, dim=1) # B x F, psi_{t'} avg over actions
        ...
        disc_target_s_features = (self.discount ** samples.done_idxs.float()).unsqueeze(1) * target_s_features
        y = features + disc_target_s_features
        # t = 98, done = 100
        # features_n = 98 + 99 , disc_target_s_features = psi(100) = phi(100) + gamma * phi(101) ... 
        # disc target_s_features (last terminal state)

        delta = y - s_features
        losses = 0.5 * delta ** 2  # B x F
        abs_delta = abs(delta)
