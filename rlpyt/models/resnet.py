...
MINIGRID_EMBEDDING = 64
MINIGRID_NORMALIZATION_ON = False
...
class MinigridNetBuilder(object):

    @staticmethod
    def build(input_shape, num_outputs, is_classification):
        ...
        conv1 = Conv2D(filters=32, kernel_size=(3, 3),
                      strides=2, kernel_initializer="he_normal")(input)
        relu1 = Activation("relu")(conv1)
        conv2 = Conv2D(filters=32, kernel_size=(3, 3),
                      strides=1, kernel_initializer="he_normal")(relu1)
        relu2 = Activation("relu")(conv2)
        flattened = Flatten()(relu2)

        last_activation = None
        if is_classification:
            last_activation = "softmax"
        dense = Dense(units=num_outputs, kernel_initializer="he_normal",
                      activation=last_activation)(flattened)
        model = Model(inputs=input, outputs=dense)
        return model

    @staticmethod
    def build_siamese(input_shape, num_outputs):
        channels, height, width = input_shape
        branch_channels = 3 #channels / 2
        branch_input_shape = (branch_channels, height, width)
        branch = MinigridNetBuilder.build(branch_input_shape, MINIGRID_EMBEDDING, False)
        ...
        relu = Activation('relu')(raw_result)

        # TODO: Add an extra layer or no?

        output = Dense(units=2, activation='softmax', kernel_initializer='he_normal')(relu)
