...
class MinigridFeatureWrapper(Wrapper):
    ...
    def __init__(self, env, num_features=64, terminate=False, reset_same=False, reset_episodes=1):
        super().__init__(env)
        self.env = env
...
def make(*args, info_example=None, mode=None, minigrid_config=None, **kwargs):
    if minigrid_config is not None:
        ...
        if mode == 'full':
            ...
            return GymEnvWrapper(MinigridFeatureWrapper(RGBImgObsWrapper(env), num_features=num_features, terminate=terminate, reset_same=reset_same, reset_episodes=reset_episodes))
